...
const BUILTIN_SLASH_COMMAND_REGISTRY: ReadonlyArray<SlashCommandSpec> = [
	...
	},
	{
		...
		acpInputHint: "[on|off|status|review|dump [raw]|configure]",
		subcommands: [
		...
		handle: async (command, runtime) => {
			...
			return usage("Usage: /advisor [on|off|status|review|dump [raw]|configure]", runtime);
		},
		handleTui: async (command, runtime) => {
			const { verb, rest } = parseSubcommand(command.args);
			if (!verb || verb === "toggle") {
			...
			runtime.ctx.showStatus("Usage: /advisor [on|off|status|review|dump [raw]|configure]");
			runtime.ctx.editor.setText("");
		},
	},
	{
